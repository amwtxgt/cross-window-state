...
```javascript
import { BaseNode, ExtensionCategory, Graph, register, Rect, Text, Circle } from '@antv/g6'
...
class MyNode extends BaseNode {
  ...
  onCreate() {
    ...
    // 启动呼吸动画
    keyShape.animate([{ r: 20 }, { r: 25 }, { r: 20 }], { duration: 2000, iterations: Infinity })
  }
...
```javascript
import { BaseEdge, ExtensionCategory, Graph, register, Path } from '@antv/g6'
...
class DashEdge extends BaseEdge {
  ...
  render(attributes, container) {
    ...
    if (keyShape) {
      ...
      keyShape.animate([{ lineDashOffset: 0 }, { lineDashOffset: -20 }], {
        duration: 1000,
        iterations: Infinity
      })
    }
...
class BreathingNode extends BaseNode {
  render(attributes, container) {
    ...
    // 错误：在 render 中启动动画
    circle.animate([{ r: 30 }, { r: 40 }, { r: 30 }], { duration: 2000, iterations: Infinity })
  }
}
...
class BreathingNode extends BaseNode {
  ...
  onCreate() {
    const keyShape = this.shapeMap['key']
    keyShape.animate([{ r: 30 }, { r: 40 }, { r: 30 }], { duration: 2000, iterations: Infinity })
  }
}
...
class BreathingCircle extends Circle {
  onCreate() {
    const halo = this.shapeMap.halo
    if (halo) {
      halo.animate([{ lineWidth: 0 }, { lineWidth: 20 }, { lineWidth: 0 }], {
        duration: 2000,
        iterations: Infinity
      })
    }
...
circle.animate([{ r: 20 }, { r: 25 }, { r: 20 }], { duration: 2000, iterations: Infinity })
```
