...
const ActionNode = ({ data, graph }) => {
  ...
  return (
    ...
    >
      ...
      {data.data.expanded && <div style={{ fontSize: 12, color: '#666' }}>{data.data.detail}</div>}
...
const ServerNode = ({ data }) => {
  ...
  return (
    <Card size="small" style={{ width: '100%', height: '100%', borderRadius: 8 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span>
          <DatabaseFilled /> {data.id}
        </span>
        <Badge status={status} />
      </div>
